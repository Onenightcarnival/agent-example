...
```text
请使用 recipe-indexer skill 帮我整理 recipe notes。
```
...
demo skill 放在 [skills/recipe-indexer/SKILL.md](skills/recipe-indexer/SKILL.md)。它的脚本是 [skills/recipe-indexer/scripts/build_recipe_index.py](skills/recipe-indexer/scripts/build_recipe_index.py)。

这个 skill 很小。它读取一份 recipe notes，生成 Markdown 索引。重点不是索引本身，而是演示 skill 可以带脚本，脚本在远程 sandbox 里执行。
...
```text
/home/gem/workspace/deepagents-cookbook/
  skills/recipe-indexer/SKILL.md
  skills/recipe-indexer/pyproject.toml
  skills/recipe-indexer/scripts/build_recipe_index.py
  skills/recipe-indexer/scripts/check.py
  skills/recipe-indexer/tests/test_build_recipe_index.py
  skills/recipe-indexer/uv.lock
  input/recipe_notes.md
  output/recipe_index.md
```
