...
DeepAgents 自带文件和命令工具。这个 recipe 会在模型调用前过滤这些内置工具，只保留显式传入的 Python tool 和 MCP tool。运行后，用户只问 agent 当前有哪些工具可用。
...
```bash
uv run --env-file .env python cookbook/02_tools_mcp/tool_mcp_integration.py
```
